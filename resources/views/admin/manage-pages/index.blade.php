...
<section class="content">
	<div class="row">
		...
		@if((date('Y-m', strtotime($_GET['date'])) == date('Y-m') || date('Y-m', strtotime($_GET['date'])) <= date('Y-m', strtotime('+1 month', strtotime(date('Y-m-d')))) )) <!-- page list -->
			<div class="col-md-12">
				<div class="box box-success">
					<div class="box-header with-border">
						<h4 class="box-title">Page List On Date: <strong>{{date("d-M-Y",strtotime($_GET['date']))}}</strong> </h4>
					</div>
					<div class="box-body">
									...
									<tr>
										...
										<td>
											<select class="form-control" name="page_number" class="form-control" required="">
												...
												@for ($i = 01; $i <= 200; $i++) <option value="{{$i}}">Page {{str_pad($i, 2, "0", STR_PAD_LEFT)}}</option>
													@endfor
											</select>
										</td>
						...
						<table class="table table-bordered table-hover table-text-center">
							...
							<tbody>
								...
								<tr>
									<td colspan="7">
										<div class="alert alert-info text-center" style="margin: 0;">No Page Uploaded Yet !</div>
									</td>
								</tr>
								@endif
							</tbody>
...
<script type="text/javascript">
	...
	*/
jQuery(function() {
    // Attach a click event handler to elements with the class 'edit_page_info'
    jQuery('.edit_page_info').click(function() {
        // Extract data attributes and values
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                // Update the content of elements with the class 'ajax_edit_page_view'
